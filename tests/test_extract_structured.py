...
class TestExtractVocab:
    ...
    def test_term_fields(self):
        cards = extract_file(VOCAB_MD)
        retainer = next(c for c in cards if "retainer" in (c.fields.get("Term") or "").lower())
    ...
    def test_fixed_price_has_slash_variants(self):
        cards = extract_file(VOCAB_MD)
        fp = next(c for c in cards if "fixed-price" in (c.fields.get("Term") or "").lower())
        assert "/" in fp.fields["Term"] or "fixed bid" in fp.fields["Term"].lower()

    def test_milestone_payment_has_why(self):
        cards = extract_file(VOCAB_MD)
        mp = next(c for c in cards if "milestone" in (c.fields.get("Term") or "").lower())
...
class TestExtractQA:
    ...
    def test_question_fields(self):
        cards = extract_file(QA_MD)
        bg = next(c for c in cards if "background" in (c.fields.get("Question") or "").lower())
    ...
    def test_industry_question_has_leadin(self):
        cards = extract_file(QA_MD)
        ind = next(c for c in cards if "industry" in (c.fields.get("Question") or "").lower())
        html = ind.fields["Answer"]
        assert "even if no" in html.lower() or "Even" in html

    def test_walkthrough_question_has_answer(self):
        cards = extract_file(QA_MD)
        walk = next(c for c in cards if "walk" in (c.fields.get("Question") or "").lower())
        assert (
            "current-state" in walk.fields["Answer"].lower()
            or "review" in walk.fields["Answer"].lower()
        )
